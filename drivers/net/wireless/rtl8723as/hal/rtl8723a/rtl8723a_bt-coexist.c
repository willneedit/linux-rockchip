...
u32 BTCoexDbgLevel = _bt_dbg_on_;
...
void btdm_1AntBtCoexistHandler(PADAPTER padapter)
{
	...
	else
	{
		...
		if (BTDM_IsWifiConnectionExist(padapter) == _TRUE)
		{
			RTPRINT(FBT, BT_TRACE, ("[BTCoex], wifi is connected   (fw state =0x%x)\n",get_fwstate(&padapter->mlmepriv)));
		...
		{
			RTPRINT(FBT, BT_TRACE, ("[BTCoex], wifi is disconnected   (fw state =0x%x)\n",get_fwstate(&padapter->mlmepriv)));
