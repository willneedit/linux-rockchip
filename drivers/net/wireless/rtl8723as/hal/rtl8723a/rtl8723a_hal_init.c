...
#if defined(CONFIG_CHECK_BT_HANG) && defined(CONFIG_BT_COEXIST)
extern void check_bt_status_work(void *pdata);
void rtl8723a_init_checkbthang_workqueue(_adapter * padapter)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,37))
	padapter->priv_checkbt_wq = alloc_workqueue("8723a_chk_bt_wq", 0, 0);
#else
	padapter->priv_checkbt_wq = create_workqueue("8723a_chk_bt_wq");
#endif
	INIT_DELAYED_WORK(&padapter->checkbt_work, (void*)check_bt_status_work);
}

void rtl8723a_free_checkbthang_workqueue(_adapter * padapter)
{
	if (padapter->priv_checkbt_wq) {
		cancel_delayed_work_sync(&padapter->checkbt_work);
		flush_workqueue(padapter->priv_checkbt_wq);
		destroy_workqueue(padapter->priv_checkbt_wq);
		padapter->priv_checkbt_wq = NULL;
	} 
}
...
{
	if (padapter->priv_checkbt_wq)
		queue_delayed_work(padapter->priv_checkbt_wq, &(padapter->checkbt_work), 0);
}
#endif