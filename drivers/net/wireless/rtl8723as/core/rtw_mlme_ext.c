...
int issue_probereq_p2p_ex(_adapter *adapter, u8 *da, int try_cnt, int wait_ms)
{
	...
	if(ret != _FAIL)
		...
		if (da)
			DBG_871X(FUNC_ADPT_FMT" to "MAC_FMT", ch:%u, limit=%d, wait_ms=%d\n",
				FUNC_ADPT_ARG(adapter), MAC_ARG(da), rtw_get_oper_ch(adapter), try_cnt, wait_ms);
		else
			DBG_871X(FUNC_ADPT_FMT", ch:%u, limit=%d, wait_ms=%d\n",
				FUNC_ADPT_ARG(adapter), rtw_get_oper_ch(adapter), try_cnt, wait_ms);
	}
	...
	int try_cnt, int wait_ms)
{
	...
	if(ret != _FAIL)
		...
		if (da)
			DBG_871X(FUNC_ADPT_FMT" to "MAC_FMT", ch:%u, limit=%d, wait_ms=%d\n",
				FUNC_ADPT_ARG(padapter), MAC_ARG(da), rtw_get_oper_ch(padapter), try_cnt, wait_ms);
		else
			DBG_871X(FUNC_ADPT_FMT", ch:%u, limit=%d, wait_ms=%d\n",
				FUNC_ADPT_ARG(padapter), rtw_get_oper_ch(padapter), try_cnt, wait_ms);
	}
